    ...
    const response = await fetch(`${import.meta.env.VITE_A4F_BASE_URL}/chat/completions`, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                ...
                {
                    role: "user",
                    content: `Give me detailed information about "${medicineName}" in simple ${language} language so that normal users can understand and in the followin format: 1. Uses, 2. Dosage (Adults and Children), 3. Common Side Effects, 4. Warnings (Pregnancy, Elderly, Liver/Kidney patients) and also keep in mind the result should not exceed more than 3000 characters so make it short and clear`
                },
            ],
        }),
    });
